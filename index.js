(function (global, factory) {
  ...
  function getRichTextFromNodes(nodes, options, key) {
    ...
    if (key === undefined) {
      return createElement(React__default.Fragment, {}, list, options);
    } else {
  ...
  function createElement(type, props, children, options) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (children && typeof children !== 'string') {
        children = React__default.Children.toArray(children);
      }
    }
    ...
    if (type === undefined) {
      ...
      return React__default.createElement(type, props, children);
    }
  }
  ...
  function memo(asyncFunc, areEqual) {
    ...
    return React__default.memo(syncFunc, areEqual);
  }

  function forwardRef(asyncFunc, areEqual) {
    var syncFunc = use(asyncFunc);
    return React__default.memo(React__default.forwardRef(syncFunc), areEqual);
  }

  function clone(element, props) {
    if (React__default.isValidElement(props)) {
      return props;
  ...
  function useEventTime() {
    ...
    var date = state[0];
    var setDate = state[1];
    var callback = React.useCallback(function (evt) {
      setDate(new Date());
    });
    React.useDebugValue(date);
  ...
  function useErrorCatcher(rethrow) {
    ...
    var run = React.useCallback(function (f) {
      try {
        var promise = f();
    ...
    var clear = React.useCallback(function (f) {
      setError(undefined);
    });
    React.useDebugValue(error);
  ...
  function useComputed(f, deps) {
    ...
    var recalc = React.useCallback(function () {
      setState({
        value: state.value
      });
    }, []);
  ...
  var prototype$1 = Object.create(React.PureComponent.prototype);
  ...
  prototype$1.render = function () {
    ...
    if (error) {
      if (parseInt(React__default.version) >= 16) {
        throw error;
      } else {
  ...
  function useSaveBuffer(params, customClass) {
    ...
    React.useDebugValue(buffer.current);
    return buffer;
  }
  ...
  function useEventProxy(deps) {
    ...
    React.useDebugValue(proxy, formatDebugValue);
    return proxy;
  }
  ...
  function getDispatcherRef() {
    if (dispatcherRef === undefined) {
      ...
      for (var name in React__default) {
        var value = React__default[name];
  ...
  function replaceChildren(node, newChildren) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      newChildren = React__default.Children.toArray(newChildren);
    }

    return React__default.cloneElement(node, undefined, newChildren);
  }
  /**

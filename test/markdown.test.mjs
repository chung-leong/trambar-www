...
describe('Markdown', function() {
    ...
    it ('should be able to retrieve test data', async function() {
        const data = await loadTestData('test-2');
    ...
    describe('MarkdownPage', function() {
        it ('should include metadata from file', async function() {
            ...
            const page = await loadTestPage('test-1');
        ...
        describe('#filter', function() {
            it ('should extract JSON data embedded in Markdown text', async function() {
                const page = await loadTestPage('test-3');
...
async function loadTestPage(name) {
...
async function loadTestData(name) {
    ...
    if (!data) {
        const res = await fetch(`${serverAddress}/wiki/${name}`);
        if (res.status !== 200) {
